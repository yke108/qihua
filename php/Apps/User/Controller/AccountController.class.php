...
//    
  ...
  /**
    ...
    public function userDetail() {
        $this->checkLogin();
        $member = D('User/Member');
        $detail = $member->detail(['id'=> $this->uid]);
        $detail = [
            'id'=> $detail['id'],
            ...
            'foxedPhone'=> $detail['foxedPhone'],
            'addressList'=> $detail['addressList'],
            'addressDetail'=> $detail['addressDetail'],
            'businessCert'=> $detail['businessCert'],
        	'img'=>picurl($detail['img']),
        ];
        $this->ajaxReturn(['detail'=> $detail]);
    }
    ...
    public function editUser() {
        if (!IS_AJAX) {
            ...
            $member = D('User/Member');
            $redis = \Think\Cache::getInstance('Redis');
            $oldDetail = $member->detail(['id'=> $this->uid]);
            ...
            $businessCert = trim(I('json.businessCert'));
            $img = trim(I('json.img'));

            $saveData = [];
            if (!empty($phone) && $phone != $oldDetail['phone']) {
                //检测手机存在
                if (!$member->IsWhitePhone(['phone'=> $phone])) {   //手机白名单
                    if ($redis->exists("string:phone:{$phone}")) throw new CsException("This Phone exists already.", 400);
                }
                $saveData['phone'] = $phone;
                $redis->del("string:phone:{$oldDetail['phone']}", $this->uid);
                $redis->set("string:phone:{$phone}", $this->uid);
            }
            if (!empty($companyName) && $companyName != $oldDetail['companyName']) {
                $saveData['companyName'] = $companyName;
            }
            if (!empty($firstName) && $firstName != $oldDetail['firstName']) {
                $saveData['firstName'] = $firstName;
            }
            if (!empty($secondName) && $secondName != $oldDetail['secondName']) {
                $saveData['secondName'] = $secondName;
            }
            if (!empty($foxedPhone) && $foxedPhone != $oldDetail['foxedPhone']) {
                $saveData['foxedPhone'] = $foxedPhone;
            }
            if (!empty($img)) {
            	$saveData['img'] = $img;
            }
            if ($areaId > 0 && $areaId != array_pop(explode(',', $oldDetail['addressList']))) {
                ...
                $saveData['addressList'] = $addressList;
            }
            if (!empty($addressDetail) && $addressDetail != $oldDetail['addressDetail']) {
                $saveData['addressDetail'] = $addressDetail;
            }
            if (!empty($businessCert) && $businessCert != $oldDetail['businessCert']) {
                $saveData['businessCert'] = $businessCert;
            }
            if (!empty($saveData)) {
                $ret = $redis->hMset('hash:member:'.$this->uid, $saveData);
                if (!$ret) {
