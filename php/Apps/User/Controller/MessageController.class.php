...
class MessageController extends CommonController{
	...
	public function sendMessage(){
		if(!IS_AJAX){
			...
			$param['from']=$this->uid;
			$param['to']=I('json.to');
			$param['subject']=I('json.subject');
			$param['content']=I('json.content');
			/*
			 *	此处需要判断接收方是否存在
			 */
			$companyName=D('User/Account')->GetNationality($param['to'],array('companyName'))['companyName'];
			if(!$companyName){
				throw new CsException("Error Processing Request", 400);
			}
			if($param['from']==$param['to']){
				throw new CsException("Error Processing Request", 400);
			}
			...
			if($res){
				...
				throw new CsException("Send failed", 1);
