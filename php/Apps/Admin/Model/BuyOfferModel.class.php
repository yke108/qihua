...
class BuyOfferModel extends Model{
    ...
    public function GetMemberCacheKeys( $uid ){
        return "hash:member:{$uid}";
    }
  public function GetAreaCacheKeys( $id ){
        ...
        if( $data['total'] && $this->redis->expire( $tmpCacheKeys,60 ) ){
            $options = array(
                'get'=>array(
                    ...
                    'hash:buyoffer:*->times','hash:buyoffer:*->state','hash:buyoffer:*->Uid','hash:buyoffer:*->location','hash:buyoffer:*->image'
                ),
                 'sort' =>'DESC',
                 'by' =>'hash:buyoffer:*->id'
            );
            ...
            if( $TmpArr ){
                foreach( $TmpArr as $k=>$v ){
                    if($k%13==0){
                        ...
                        $listArr[$num]['location']=empty($v)?'':$v;
                    }elseif($k%13==12){
                        $listArr[$num]['image']=empty($v)?'':$v;
                        $num++;
                    }
                }
            }
           ...
           foreach($listArr as $k1=>$v1){
               ...
               if( $listArr[$k1]['state'] == 0 || $listArr[$k1]['state'] == 4 ){
                   $reason = $this->getOneHistory($listArr[$k1]['id'])['reason'];
                   $stateTip .= "(原因：'{$reason}')";
               }
               $listArr[$k1]['stateTip'] = $stateTip;
    ...
    public function GetMemberInfo( $id,$pram='' ){
        ...
        $cacheKeys=$this->GetMemberCacheKeys( $id );
        if( !empty( $pram ) ){
            $ret = $this->redis->hmget( $cacheKeys,$pram );
        }else{
            $ret = $this->redis->hgetAll( $cacheKeys );
