...
//        array('group','number','所属角色格式不正确！'),
    ...
    public function getaccountinfo($username,$password,$salt){
        ...
        $map['status'] = 1;
        $map['state'] = 1;
        $status=$this->field('id,username,lastlogintime,realname,department,group')->where($map)->find();//查询登录状态
        if(false===$status){
            return false;
        }else{
            return $status;
    ...
    protected  function checkpasswd($password,$username){
        ...
        $field='salt, password';
        $result = $this->getUserData($field,$where);
        $password = $this->passencrypt($password,$result['salt']);
