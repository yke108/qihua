...
class DataController extends ContentController{
	...
	public function getCategory(){
		$ret = D('Category')->getCategory();
		$ret = $this->switchLinkageData($ret);
		$this->ajaxReturn(['data'=> $ret]);
	}
	...
	public function modelList(){
		$ret = D('Companydata')->getList('model');
		$this->ajaxReturn(['rows'=> $ret[0]['children']]);
	}
	//经营模式添加
...
//		}
   ...
   * */
    ...
    public function indicatorList() {
        $page = (int)I('json.page', 1);
        $rows = (int)I('json.rows', 20);
        $keyword = trim(I('json.keyword'));
        if (IS_AJAX && $keyword) {
