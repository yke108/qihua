...
//                );
    ...
    public function addProduct() {
        if (IS_POST) {
            ...
            // sku数据
            if (!isset($data['specification']) || !is_array($data['specification']) || count($data['specification']) < 1) {
                $ret['code'] = 400;
                $ret['msg'] = '请输入规格信息';
                $this->ajaxReturn( $ret );
            }
            $skuList = [];
            for ($i = 0; $i < count($data['specification']); $i++) {
                $specification = isset($data['specification'][$i]) ? $data['specification'][$i] : '';
                $price = isset($data['price'][$i]) ? (float)$data['price'][$i] : 0;
                $inventoryType = isset($data['inventoryType'][$i]) ? (int)$data['inventoryType'][$i] : 1;
                $inventory = isset($data['inventory'][$i]) ? (int)$data['inventory'][$i] : 2;
                $inventoryNum = isset($data['inventoryNum'][$i]) ? (int)$data['inventoryNum'][$i] : 0;
                $moq = isset($data['moq'][$i]) ? (int)$data['moq'][$i] : 0;
                $weightUnit = isset($data['weightUnit'][$i]) ? (int)$data['weightUnit'][$i] : 1;
                $packWeigth = isset($data['packWeigth'][$i]) ? (float)$data['packWeigth'][$i] : 0;
                if (empty($specification) || $price <= 0 || !in_array($inventoryType, [1,2]) || !in_array($inventoryType, [1,2]) || $moq < 1 || $packWeigth <= 0) {
                    $ret['code'] = 400;
                    $ret['msg'] = '规格信息填写不完整';
                    $this->ajaxReturn( $ret );
                }
                ...
                if ($i == 0) {
                    ...
                    $data['inventoryNum'] = $inventoryNum;
                    $data['moq'] = $moq;
