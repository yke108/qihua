...
function picurl($path, $thumb = ''){
	...
	if(strpos($path, '://') > 0){
		...
		return 'http://kwen.t.weixinren.cn'.$path;
	}
	if ($thumb) {
		return 'http://kwen.t.weixinren.cn/Uploads'."/thumbs/".$thumb.'/'.$path;
	}else {
		return 'http://kwen.t.weixinren.cn/Uploads'.'/'.$path;
	}
}
...
function ppic($path){
	...
	return 'http://kwen.t.weixinren.cn/Public/'.$path;
}
/**
 * 公共函数
 */
...
/*发邮件*/
function send_mail($to,$title,$content,$echo=true){
...
function getcategory(){
	$cateList = D('Category')->getcategorycache();
    $product = D('Product');
    ...
    foreach ($cateList as $value){
        $cate = ['id'=> $value['id'], 'text'=> $value['text'], 'url'=>U('product/index', ['categoryId'=>$value['id']])];
        foreach ($value['children'] as $v1) {
            $cate1 = ['id'=> $v1['id'], 'text'=> $v1['text'], 'url'=>U('product/index', ['categoryId'=>$v1['id']])];
            foreach ($v1['children'] as $v2) {
                $cate1['children'][] = ['id'=> $v2['id'], 'text'=> $v2['text'], 'url'=>U('product/index', ['categoryId'=>$v2['id']])];
            }
            $cate['children'][] = $cate1;
        }
        $list = $product->lists(array('p'=>0,'page_size'=>2,'state'=>1,'category'=>$value['id']));
        $products = [];
        if (!empty($list['lists'])) {
            foreach ($list['lists'] as $l){
                $products[] = [
                    'id' => $l['id'],
                    'title' => $l['title'],
                    'currency' => empty($currency[$l['currency']]['character']) ? '' : $currency[$l['currency']]['character'],
                    'price' => sprintf('%.2f', $l['price']),
                    'weightUnit' => $weightUnit[$l['weightUnit']]['name'],
                    'thumb' => picurl(unserialize($l['images'])[0]),
                	'url'=>U('/product/detail',['id'=>$l['id']]),
                ];
            }
        }
        if (!empty($products)) {
