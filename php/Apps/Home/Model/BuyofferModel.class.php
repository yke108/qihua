...
class BuyofferModel extends Model{
	...
	public function __construct(){
		...
		public function detailList($pageSize=6){
			...
			if($tmpset &&$this->redis->expire('tmp:set:buyoffer:list:'.$tmp,60)){
				$uid=$this->redis->sort('tmp:set:buyoffer:list:'.$tmp,array('get'=>array('hash:buyoffer:*->Uid'),'by'=>'hash:buyoffer:*->updateTime','sort'=>'desc','limit'=>array($offset,$pageSize)));
	...
	public function indexlist($param){
		...
		//条件：有效，未删除
		$pageSize = isset($param['pageSize']) ? (int)$param['pageSize'] : 5;
		...
		if(!empty($param['type'])){
			...
			$t=uniqid();
			$this->redis->zUnion("tmp:set:buyoffer:Union:".$t,$tarr);
			$arr[]="tmp:set:buyoffer:Union:".$t;
		}
		$arr[]=$state=$this->GetkeyState(1);
		if(!empty($param['country']))$arr[]=$country=$this->GetKeyCountry($param['country']);
		if(!empty($param['title'])){
			$filter['title']=$param['title'];
			$arr[]=$title=$this->shell->search('buyoffer:title',strtolower($param['title']),'set');
		}
		...
		if($tmpset && $this->redis->expire('tmp:set:buyoffer:list:'.$tmp,60)){
			...
			$title=$this->redis->sort('tmp:set:buyoffer:list:'.$tmp,array('get'=>array('hash:buyoffer:*->title'),'by'=>'hash:buyoffer:*->updateTime','sort'=>'desc','limit'=>array($offset,$pageSize)));
			$type=$this->redis->sort('tmp:set:buyoffer:list:'.$tmp,array('get'=>array('hash:buyoffer:*->type'),'by'=>'hash:buyoffer:*->updateTime','sort'=>'desc','limit'=>array($offset,$pageSize)));
            $location=$this->redis->sort('tmp:set:buyoffer:list:'.$tmp,array('get'=>array('hash:buyoffer:*->location'),'by'=>'hash:buyoffer:*->updateTime','sort'=>'desc','limit'=>array($offset,$pageSize)));
            $image=$this->redis->sort('tmp:set:buyoffer:list:'.$tmp,array('get'=>array('hash:buyoffer:*->image'),'by'=>'hash:buyoffer:*->updateTime','sort'=>'desc','limit'=>array($offset,$pageSize)));
			$mold=C("FIND_GOODS_TYPE");
			$j=count($title);
			for($i=0;$i<$j;$i++){
				$list[$i]['id']=$id[$i];
				if (empty($location[$i])) {
					$addressList=D('User/Account')->GetNationality($uid[$i], array('addressList'))['addressList'];
					$addressList = explode(',', $addressList);
					rsort($addressList);
					foreach ($addressList as $key => $value) {
						$addressList[$key] = getAreaName($value);
					}
					$list[$i]['location'] = implode(' ', $addressList);
				} else{
					$list[$i]['location']=$location[$i];
				}
				$list[$i]['title']=$title[$i];
				$list[$i]['image'] = empty($image[$i]) ? '' : $image[$i];
				$list[$i]['type']=$mold[$type[$i]];
			}
		}
		return ['count'=> (empty($count) ? 0 : $count), 'list'=> $list];
	...
	public function details($param){
		...
		if (!empty($res)) {
			if (empty($res['location'])) {
				$addressList=D('User/Account')->GetNationality($res['Uid'], array('addressList'))['addressList'];
				$addressList = explode(',', $addressList);
				rsort($addressList);
				foreach ($addressList as $key => $value) {
					$addressList[$key] = getAreaName($value);
				}
				$res['location'] = implode(' ', $addressList);
			}
		}
		return $res;
	}
	...
	public function showpage($count,$page,$pageSize=6,$filter='',$showPage=5){
		//计算总页数
