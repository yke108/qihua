...
class ProductModel extends CommonModel{
    ...
    public function lists( $param ){
        ...
        if( $result && $this->redis->expire( $tempCacheKey, 60 ) ){
            $array = array(
                'get' => array(
                    'hash:product:*' => array(
                        'id', 'productCode', 'title', 'price', 'weightUnit', 'currency', 'moq', 'inventory','inventoryType','inventoryNum', 'paymentMethod', 'logisticsMethod', 'addTime', 'updateTime', 'lastUpdateIp', 'sales', 'tradingCapacity', 'state', 'images', 'cas', 'categoryList', 'seatList','brandId', 'producerId','attribute','skuId'
                    )
                ),
    ...
    public function insert( $data ){
        ...
        if( $valid ){
            ...
            $cacheKey = $this->getStateCacheKey( $states['ACTIVE']['value'] );
            ...
            $this->redis->sadd( $cacheKey, $id );// 经营模式和商品绑定
     ...
     * @return boolean
     */
    public function edit( $id, $data, $oldData ){
        $ret = false;
        $id = intval( $id );
        if( empty( $id ) ){
            return $ret;
        ...
        if( $valid ){
            ...
            if ($uid == 0) {
                if( $oldData['attribute']['model'] != $data['model'] ){
                    $cacheKey = $this->getModelCacheKey( $oldData['attribute']['model'] );
                    $this->redis->srem( $cacheKey, $id );//关联商品ID
        ...
        if( $valid ){
            ...
            if( $ret ){
                ...
                if( isset( $data['moq'] ) || isset( $data['inventory'] ) ){
                    ...
                    if( $newData['inventoryType'] == 2 ){
                        $stockCacheKey = $this->getStockCacheKey( $stockStatus['OUT_OF_STOCK']['value'] );
                        $this->redis->sadd( $stockCacheKey, $id );//库存状态
                    }elseif( $newData['inventory'] == 1 && $newData['inventoryNum'] < $newData['moq'] ){
                        $stockCacheKey = $this->getStockCacheKey( $stockStatus['LOW_STOCK']['value'] );
                        $this->redis->sadd( $stockCacheKey, $id );//库存状态
                    }
     ...
     * @return string
     */
    public function getProductWeightUnit($param=''){
        if (!empty($param)) {
            return C('WEIGHTUNIT')[$param]['name'];
        }
        return C('WEIGHTUNIT');
